...
def minmod(a, b):
    """
    Compute the minimum in modulus of the arguments for limiting
...
def minmod_rusanov_step(q, g, flux):
    ...
    for i in range(g.ngz, g.nx + g.ngz + 1):
        f_L = flux(q_L[i])
        f_R = flux(q_R[i])
        f_rusanov[i] = (f_L + f_R + g.dx / g.dt * (q_L[i] - q_R[i])) / 2
...
def minmod_rusanov(nx, t_end, system,
    ...
    while t < t_end:
        ...
        t += g.dt
        # Take a single step
        q += g.dt * minmod_rusanov_step(q, g, flux=system.flux)
...
if __name__ == "__main__":
    ...
    x, q = minmod_rusanov(20, 1.0, system, bc_form="periodic")
    ...
    pyplot.ylabel(r"$q$")
    pyplot.legend()
    pyplot.title("Minmod, t=1")
    pyplot.show()
    
    # Solve for ten periods to see phase and dissipation errors
    x, q = minmod_rusanov(20, 10.0, system, bc_form="periodic")
    x_exact = numpy.linspace(0, 1, 1000)
    q_exact = initial_data_exp_sine(x_exact)
    pyplot.plot(x_exact, q_exact, 'k-', label="Exact")
    ...
    for i, N in enumerate(Ns):
        x, q = minmod_rusanov(N, 1.0, system, bc_form="periodic")
    ...
    for t_end, fmt in zip(Ts, mpl_formats):
        x, q = minmod_rusanov(nx, t_end, system, initial_data=initial_data_exp_sine)
        pyplot.plot(x, q, fmt, label=fr"$t={t_end}$")
    pyplot.xlabel(r"$x$")
    pyplot.ylabel(r"$q$")
    pyplot.legend()
    pyplot.title("Minmod-Rusanov, Burgers, nx=200")
    pyplot.show()
    ...
    for nx, fmt in zip(Ns, mpl_formats):
        x, q = minmod_rusanov(nx, 0.2, system, initial_data=initial_data_exp_sine)
        pyplot.plot(x, q, fmt, label=fr"$N={nx}$")
    pyplot.xlabel(r"$x$")
    pyplot.ylabel(r"$q$")
    pyplot.legend()
    pyplot.title("Minmod-Rusanov, Burgers, t=0.2")
    pyplot.show()
    
    # Repeat for the hat initial data to see the wave structure
    for nx, fmt in zip(Ns, mpl_formats):
        x, q = minmod_rusanov(nx, 0.2, system, initial_data=initial_data_hat)
        pyplot.plot(x, q, fmt, label=fr"$N={nx}$")
    pyplot.xlabel(r"$x$")
    pyplot.ylabel(r"$q$")
    pyplot.legend()
