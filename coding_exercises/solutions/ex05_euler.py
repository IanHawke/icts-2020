...
def godunov_rusanov_step(q, g, flux):
    ...
    for i in range(g.ngz, g.nx + g.ngz + 1):
        f_rusanov[i, :] = (f[i-1, :] + f[i, :] + g.dx / g.dt * (q[i-1, :] - q[i, :])) / 2
...
def godunov_hlle_step(q, g, flux, c2p=None):
    ...
    for i in range(g.ngz, g.nx + g.ngz + 1):
        ...
        xi_m_L = v[i-1] - cs[i-1]
        xi_m_R = v[i] - cs[i]
        xi_hat_plus = max([xi_p_L, xi_p_R, 0])
        xi_hat_minus = min([xi_m_L, xi_m_R, 0])
        f_HLLE[i, :] = (xi_hat_plus * f[i-1, :] - xi_hat_minus * f[i, :] - 
                        xi_hat_plus * xi_hat_minus * (q[i-1, :] - q[i, :])) / (xi_hat_plus - xi_hat_minus)
