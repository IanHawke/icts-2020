"""
Exercise 3: Burgers equation with the upwind scheme
"""
...
if __name__ == "__main__":
    ...
    for t_end, fmt in zip(Ts, mpl_formats):
        x, q = upwind(nx, t_end, system, initial_data=initial_data_exp_sine)
        pyplot.plot(x, q, fmt, label=fr"$t={t_end}$")
    pyplot.xlabel(r"$x$")
    pyplot.ylabel(r"$q$")
    pyplot.legend()
    pyplot.title("Upwind, Burgers, nx=200")
    pyplot.show()
    ...
    for nx, fmt in zip(Ns, mpl_formats):
        x, q = upwind(nx, 0.2, system, initial_data=initial_data_exp_sine)
        pyplot.plot(x, q, fmt, label=fr"$N={nx}$")
    pyplot.xlabel(r"$x$")
    pyplot.ylabel(r"$q$")
    pyplot.legend()
    pyplot.title("Upwind, Burgers, t=0.2")
    pyplot.show()
    
    # Repeat for the hat initial data to see the wave structure
    for nx, fmt in zip(Ns, mpl_formats):
        x, q = upwind(nx, 0.2, system, initial_data=initial_data_hat)
        pyplot.plot(x, q, fmt, label=fr"$N={nx}$")
    pyplot.xlabel(r"$x$")
    pyplot.ylabel(r"$q$")
    pyplot.legend()
